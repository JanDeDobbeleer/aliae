<#
.SYNOPSIS
    Builds the MSIX package for Aliae.

.DESCRIPTION
    This script creates the MSIX installer package for Aliae with the specified architecture and version.
...
function Invoke-PackageSigning {
    ...
    try {
        ...
        & $SigningTools.SignTool sign /v /debug /d "Aliae" /fd SHA256 /tr 'http://timestamp.acs.microsoft.com' /td SHA256 /dlib $SigningTools.SignToolDlib /dmdf ../../src/metadata.json $PackagePath
...
if ($Copy) {
    $sourceFile = switch ($Architecture) {
        'x64' { "aliae-windows-amd64.exe" }
        Default { "aliae-windows-$Architecture.exe" }
    }

    Write-Verbose "Copying $sourceFile to ./dist/aliae.exe" -Verbose
    ...
    try {
        ...
        Copy-Item -Path $sourcePath -Destination "./dist/aliae.exe" -Force
    }
    catch {
